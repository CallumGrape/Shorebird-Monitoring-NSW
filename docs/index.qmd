...
for (species_name in names(plots_list)) {
  filename <- paste0("index_files/", gsub(" ", "_", species_name), ".jpeg")
...
::: panel-tabset
```{r 5 display-plots, echo = FALSE, eval = TRUE, results = 'asis'}
# List all saved plot files
plot_files <- list.files('index_files', pattern = '\\.jpeg$', full.names = TRUE)

# Display each plot in a tab
for (file in plot_files) {
  species_name <- tools::file_path_sans_ext(basename(file)) %>%
    gsub("_", " ", .)
  cat("\n\n### ", species_name, "\n\n")
  cat("![", species_name, "](", basename(file), ")\n\n")
}
```
:::
