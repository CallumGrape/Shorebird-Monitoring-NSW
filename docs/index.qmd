...
```{r 3 packages, message = FALSE, warning = FALSE, eval = TRUE}
library(dplyr)
library(here)
library(ggplot2)
library(lubridate)
...
```{r 4 avsu packages, message = FALSE, warning = FALSE, eval = TRUE}
library(dplyr)
library(here)
library(forcats) 
library(ggplot2)
...
# Tide
tide_data <- readr::read_csv(here::here("1_data", "tides", "TideDataNewcastle.csv"))
...
To compare the time available in each category (high tide day, high tide night, low tide day, and low tide night) against the time shorebird species spend at each MOTUS station in our local array, we first need to determine the available time for each species group. We also need to identify which time periods for each category were actually recorded at each station.

Below, based on [New South Wales Tide Tables](https://www.bom.gov.au/oceanography/projects/ntc/nsw_tide_tables.shtml) we were able to extract and classify the total time for each of the four categories of tide that occured within the Hunter estuary. Note that one dataset is provided at the estuary scale, therefore, we assume the tide is occurring at the same scale, time, frequency and amplitude across our MOTUS station.

Let's first clean and make tide data ready for further analysis.
...
Now tide data able to be processed, we need to define for each individual (birds) and across each species, what was the amount of time for each of the four tide categories, available to them during the time they were tagged and surveyed.
This, will be determined based on the tide data we just processed, put against the total period of survey effort for each birds.

Below, we first provide for every individual, the total amount of time (*used*) spent at each MOTUS during the four tide categories. This allow us to see the variability of "*where*" and "*how long*" shorebird species population have been using the tide across time (nb of hours) and space (MOTUS stations).  
...
Now the *used* determined, we need to define the *available*:  

To do so, we need to extract for each individual, the period that covered its survey:  from the day it was tagged to its last detection. This allows us to calculate (sum) the total amount of time that occurred for each of the four tide categories, regardless the *use* of the bird. 

```{r 4 Available vs Used TIDE AVAILABLE TIME, message = FALSE, warning = FALSE, echo = TRUE, eval = TRUE, results = 'asis'}
...
# Extract for each indiv. the total time of the 4 tide cat.
results_list <- list()
...
# Combine all species summaries
tide_data_plot <- bind_rows(results_list) %>%
  rename(Band.ID = bird_id)
```
  
Now we determined the total period for the four tide categories that occurred within the Hunter estuary, we need to clarify at MOTUS station level the survey effort. Indeed, since the survey period for each MOTUS station when it is actually activated ("switched on") is unequal across the array because of different time of deployment and different failing issues, the period of time for each tide categories may differ along the temporal coverage at station scale.
To reflect this *survey effort* variability across MOTUS station we must first determine when each station was ON/OFF, then group the complete tide categories period (`tide_data_plot`) per individual (`Band.ID`) and station (`recvDeployName`), to subtract the period where a station was actually "switched OFF" and keep only the period a station could potentially detect a tag. 
  
Thus, the method allows a fair determination of *available* time at each station for each individuals, reflecting the survey effort from station and the temporal coverage of each individuals.
  
```{r 4 Available vs Used TIDE AVAILABLE STATION BIAS, message = FALSE, warning = FALSE, echo = TRUE, eval = TRUE, results = 'asis'}
...
Now the survey effort at each MOTUS station taken into account to determine *available* time (above), we can visually display the results.  

Let's plot the results below. See below one tab per category of tide and displayed the time resource available versus used for each species of shorebirds and across our stations within the Hunter estuary.
...
plots <- purrr::map(tide_categories, function(tc) {
  ...
  p <- ggplot(plot_df, aes(x = recvDeployName, y = duration_h_num, alpha = dataset)) +
    ...
    # Boxplot for each recv, with at first box the total period of time at this recv
