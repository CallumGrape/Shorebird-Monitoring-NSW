...
uptime_summary <- recv_hours %>%
  ...
  mutate(cont_listening_eff = round(100 * operational_hours / sum(operational_hours), 1), # % of each station surv eff regarding the total(all station) of survey effort
         surv_time_cover = round(100 * operational_hours / max(total_hours),1)) %>% # % of each station surv eff regarding the time coverage of the all stations
...
***Legend:*** *The band in grey represents the operational period for each station, when the antenna is actually listening. When no grey, the station is "OFF"*
