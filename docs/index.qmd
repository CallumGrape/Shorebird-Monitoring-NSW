...
Since 2021, we trapped `r moni$value[moni$metric == "nb_tagged"]` birds, and `r moni$value[moni$metric == "nb_released"]` were released and potentially detectable across our MOTUS array.
...
Since 2021, we trapped `r moni$value[moni$metric == "nb_tagged"]` birds, and `r moni$value[moni$metric == "nb_released"]` were released and potentially detectable.
...
ggplot(tag_detection %>%
  ...
  labs(x = "Band ID",
       y = "Detection (%)",
       title = "Bird detection across MOTUS array (%)")
```

#### Per species

```{r 3 plot sp, echo = TRUE, eval = TRUE}
ggplot(tag_detection %>%
         group_by(speciesEN) %>%
