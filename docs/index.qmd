...
This is below our global coverage from our array, reflected by the number of hours every species have been recorded within the estuary and (II) the rate of detection for each species taking into account the period every birds were detectable (station operational *vs* bird tagged).

```{r back hand 21_script copied pasted to save time, message = FALSE, warning = FALSE, eval = TRUE, echo = FALSE}}
...
###I. Total record of detection from our tagged birds in our MOTUS array

```{r total hours recorded,  message = FALSE, warning = FALSE, eval = TRUE, echo = TRUE}
ggplot(balance_table, aes(x = tideCategory, y = total_used_t, fill = tideCategory)) +
...
```
###II. Global use of the array from our tagged birds

```{r total rate use,  message = FALSE, warning = FALSE, eval = TRUE, echo = FALSE}

ggplot(balance_table, aes(x = tideCategory, y = total_rate_use, fill = tideCategory)) +
  geom_boxplot(outlier.shape = NA) +
  facet_wrap(~speciesEN) +
...
```{r 03 table1 backhand code, message = FALSE, warning = FALSE, eval = TRUE, echo = FALSE}
...
```{r 2 filter the noise plot things, message = FALSE, warning = FALSE, eval = TRUE, echo = FALSE}
# Load local data
...
ggpubr::ggexport(plot10,
                 filename = here("figures", "motus_filter_count.jpg"),
...
names(plots_list) <- c("BTG", "FEC", "ML", "PGP", "PS", "RNA", "W")
...
### Whimbrel
<img src="figures/W.jpg" style="max-width:100%;">
